...
Biomass measured were taken the same day as pre trial crop circle 13/7/2022
...
Crop circle and Phenom survey undertaken by Damian on 13/7/2022
...
- `"\\FSSA2-ADL\clw-share1\Microlab\VF\Pinnaroo 2022\Biomass Data Maps\Raw_data\Phenom 2022\Pre trial\merged Phenom Pinnaroo 2022.csv"`
- `"\\FSSA2-ADL\clw-share1\Microlab\VF\Pinnaroo 2022\Biomass Data Maps\Raw_data\CC 2022\Pre trial\merged CC Pinnaroo 2022.csv"`
...
- `"\\FSSA2-ADL\clw-share1\Microlab\VF\Pinnaroo 2022\Biomass Data Maps\CC_PreTrial\DIST\Vesper\mergedCCPinnaroo2_HighDensity_DIST_PRED_2m.tif"`
- `"\\FSSA2-ADL\clw-share1\Microlab\VF\Pinnaroo 2022\Biomass Data Maps\CC_PreTrial\LAI_VI\Vesper\mergedCCPinnaroo2_HighDensity_LAI_VI_PRED_2m.tif"`
- `"\\FSSA2-ADL\clw-share1\Microlab\VF\Pinnaroo 2022\Biomass Data Maps\CC_PreTrial\NDVI\Vesper\mergedCCPinnaroo2_HighDensity_NDVI_PRED_2m.tif"`
...
- `"\\FSSA2-ADL\clw-share1\Microlab\VF\Pinnaroo 2022\Biomass Data Maps\Phen_PreTrial\DIST\Vesper\mergedPhenomPinnar_HighDensity_DIST_PRED_2m.tif"` 
- `"\\FSSA2-ADL\clw-share1\Microlab\VF\Pinnaroo 2022\Biomass Data Maps\Phen_PreTrial\LAI_VI\Vesper\mergedPhenomPinnar_HighDensity_LAI_VI_PRED_2m.tif"` 
- `"\\FSSA2-ADL\clw-share1\Microlab\VF\Pinnaroo 2022\Biomass Data Maps\Phen_PreTrial\NDVI\Vesper\mergedPhenomPinnar_HighDensity_NDVI_PRED_2m.tif"` 
...
include_graphics("W:/VF/Pinnaroo 2022/Biomass Data Maps/maps/Biomass_maps_Pre_all_grids.jpg")
...
Need to confirm the sampling size I have assumed 2*1m cuts with 30cm row spacing
...
pre_trial_biomass <- read.csv("W:/VF/Pinnaroo 2022/Biomass Data Maps/calibration_pre_trial_biomass/pre_trial_pts_extract_grids_withBiomass.csv")
                             
                             
#names(no_trial_biomass)
#rename some clm this is the order that was extered when extrcated in Arcmap
pre_trial_biomass <- pre_trial_biomass %>%
  rename(
    Survey_Phen_DIST = Phen_DIST,
    Survey_Phen_LAI = Phen_LAI,
    Survey_Phen_NDVI = Phen_NDVI,
...
pre_trial_biomass_long_CC_NDVI_no <- pre_trial_biomass_long %>% 
  filter(survey_type == "Survey_CC_NDVI")
pre_trial_biomass_long_CC_LAI_no <- pre_trial_biomass_long %>% 
  filter(survey_type == "Survey_CC_LAI")
pre_trial_biomass_long_CC_DIST_no <- pre_trial_biomass_long %>% 
  filter(survey_type == "Survey_CC_DIST")

pre_trial_biomass_long_Phen_NDVI_no <- pre_trial_biomass_long %>% 
  filter(survey_type == "Survey_Phen_NDVI")
pre_trial_biomass_long_Phen_LAI_no <- pre_trial_biomass_long %>% 
  filter(survey_type == "Survey_Phen_LAI")
pre_trial_biomass_long_Phen_DIST_no <- pre_trial_biomass_long %>% 
  filter(survey_type == "Survey_Phen_DIST")



cor_no_CC_NDVI <- cor(pre_trial_biomass_long_CC_NDVI_no$value, 
                       pre_trial_biomass_long_CC_NDVI_no$kg_ha)
cor_no_CC_LAI <- cor(pre_trial_biomass_long_CC_LAI_no$value, 
                      pre_trial_biomass_long_CC_LAI_no$kg_ha)
cor_no_CC_DIST <- cor(pre_trial_biomass_long_CC_DIST_no$value, 
                       pre_trial_biomass_long_CC_DIST_no$kg_ha) 

cor_no_Phen_NDVI <- cor(pre_trial_biomass_long_Phen_NDVI_no$value, 
                         pre_trial_biomass_long_Phen_NDVI_no$kg_ha)
cor_no_Phen_LAI <- cor(pre_trial_biomass_long_Phen_LAI_no$value, 
                        pre_trial_biomass_long_Phen_LAI_no$kg_ha)
cor_no_Phen_DIST <- cor(pre_trial_biomass_long_Phen_DIST_no$value, 
                         pre_trial_biomass_long_Phen_DIST_no$kg_ha) 
...
model_no_CC_NDVI <- summary(lm(kg_ha ~ value, 
                                data = pre_trial_biomass_long_CC_NDVI_no))
model_no_CC_LAI <- summary(lm(kg_ha ~ value, 
                               data = pre_trial_biomass_long_CC_LAI_no))
model_no_CC_DIST <- summary(lm(kg_ha ~ value, 
                                data = pre_trial_biomass_long_CC_DIST_no))

model_no_Phen_NDVI <- summary(lm(kg_ha ~ value, 
                                  data = pre_trial_biomass_long_Phen_NDVI_no))
model_no_Phen_LAI <- summary(lm(kg_ha ~ value, 
                                 data = pre_trial_biomass_long_Phen_LAI_no))
model_no_Phen_DIST <- summary(lm(kg_ha ~ value, 
                                  data = pre_trial_biomass_long_Phen_DIST_no))
...
The Crop Circle LAI has the best correlations (0.79) but the R2 is 0.62.


`summary(lm( kg_ha~  value, data = pre_trial_biomass_long_CC_LAI_no))`
...
biomass = -191	 +  1786.4 * CC  LAI survey value



### Grids of biomass THIS IS STILL TO COME
...
include_graphics(""W:/VF/Pinnaroo 2022/Biomass Data Maps/Pre trial_biomass_LAI.png"")
